...
exports.insert = (tablename, userObj, result) => {
...
exports.select = (databaseTable, result) => {
  sql.query(`SELECT * FROM ${databaseTable} ORDER BY id DESC`, (err, output) => {
    if (err) return result(err, null);
    return result(null, output);
  });
};
...
exports.selectByOne = (databaseTable, column, selectValue, result) => {
  sql.query(`SELECT * FROM ${databaseTable} WHERE ${column} = ? order by id desc`, [selectValue], (err, output) => {
    if (err) {
...
exports.selectByTwo = (table, col1, col2, column, selectValue, result) => {
  sql.query(`SELECT ${col1}, ${col2} FROM ${table} WHERE ${column} = ?`, [selectValue], (err, output) => {
...
exports.selectByMany = (tableName, innerDB, tableColumn, innertableColumn, selectValue, result) => {
  sql.query(`SELECT *, ${tableName}.id as id  FROM ${tableName} INNER JOIN ${innerDB} ON(${tableName}.${tableColumn}=${innerDB}.${innertableColumn}) WHERE ${tableName}.${tableColumn} = ? ORDER BY ${tableName}.id DESC`, [selectValue], (err, output) => {
    if (err) return console.log(err), result(err, null);
    return result(null, output);
  });
};

exports.update = async (table, tableData, columnName, id, result) => {
  await sql.query(
    `UPDATE ${table} SET ? WHERE ${columnName} = ?`,
    [tableData, id],
    (err, res) => {
      if (err) {
...
exports.alterTable=(tableName, columu, pgstatus, result)=>{
  sql.query(`ALTER TABLE ${tableName} ADD ${columu} INT(11) NULL DEFAULT '0' AFTER ${pgstatus}`, (error, res)=>{
    if (error) return console.log(error), result(error, null);
    return result(null, res);
  })
}
...
exports.updateById = (id, fullbag, halfbag, quarterbag, result) => {
  sql.query(
    "UPDATE product SET fullbag = ?, halfbag = ?, quaterbag = ? WHERE id = ?",
    [fullbag, halfbag, quarterbag, id],
    (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(null, err);
